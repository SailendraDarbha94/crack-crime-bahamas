...
const Page = () => {
  ...
  const fileSaver = async () => {
    if (selectedFile) {
      const storageRef = ref(storage, `/adverts/${selectedFile.name}`);
  ...
  const advertChanger = async () => {
    ...
    try {
      ...
      const newKey = await push(child(dbRef(db), 'adverts')).key;
      console.log(newKey)
      const updates: any = {};
      updates["/adverts/" + newKey] = {
        group: "home",
  ...
  return (
    <div className="w-full font-nunito">
      ...
      <div className="w-1/2 bg-sky-300 p-2 rounded-xl">
        <p>Home screens</p>
        {loading ? (
          <div
            role="status"
          ...
          <div>
            ...
            <button
              ...
              className="bg-emerald-500 rounded-lg block mx-auto p-2 font-extrabold"
            >
              UPLOAD
            </button>
          </div>
