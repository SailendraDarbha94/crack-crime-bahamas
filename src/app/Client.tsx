...
import { Button, NextUIProvider, Spacer } from "@nextui-org/react";
...
export default function Client({ children }: { children: React.ReactNode }) {
  ...
  return (
    <NextUIProvider>
      <div>
        <div
          className={`min-w-screen flex flex-col w-full border-none z-50 transition-height fixed top-0 left-0 bg-gradient-to-tr from-teal-50 to-teal-400 p-2 hover:cursor-pointer ${navbarHeight}`}
        >
          <div className="flex justify-between w-full">
            <Button
              radius="full"
              className={`bg-gradient-to-tr from-purple-400 to-blue-900 text-white shadow-lg mx-auto`}
              onPress={() => {
                router.push("/");
              }}
            >
            ...
            <Button
              radius="full"
              className={`bg-gradient-to-tr from-gray-500 to-gray-900 text-white shadow-lg mx-auto`}
              onPress={() =>
                router.back()
              }
            >
